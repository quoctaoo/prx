...
https://app.homeproxy.vn/api/v1/users/rotatev2?token=xxx"""
)
...
def get_proxy(url):
    try:
        ...
        # homeproxy / api khác
            ...
            if "proxy" in data["data"]:
                proxy = data["data"]["proxy"]

        elif "result" in data and isinstance(data["result"], dict):
            if "proxy" in data["result"]:
                proxy = data["result"]["proxy"]

        # thời gian chờ
        wait_time = (
            data.get("next_allowed_in_seconds")
            or data.get("nextRotate")
            or data.get("cooldown")
            or cooldown
        )

        return proxy, int(wait_time)
